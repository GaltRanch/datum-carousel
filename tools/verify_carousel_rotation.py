...
Rule (datum_blocktemplates.c):
    set      = fresh supplier addresses (template.previousblockhash == prevhash), sorted bytewise
...
def expected_pick(prevhash: str, addrs, cycle: int, skipped: int = 0) -> str:
    n = len(addrs)
    return addrs[(start_for(prevhash, n) + cycle + skipped) % n]


def fresh_set_from_dir(template_dir: str, prevhash: str):
    out = []
    for fn in os.listdir(template_dir):
        ...
        try:
            ...
            continue
        if t.get("previousblockhash") == prevhash:
            out.append(fn[:-5])
    return sorted(out)
...
def check_api(url: str, template_dir: str | None) -> bool:
    ...
    n, addrs = st["n"], st["set"]
    print(f"height={st['height']} prevhash={st['prevhash']} cycle={st['cycle']} n={n} start={st['start']} skipped={st['skipped']} idx={st['idx']} pick={st['pick']}")
    if n == 0:
    ...
    exp = expected_pick(st["prevhash"], addrs, st["cycle"], st["skipped"])
    if st["idx"] >= 0 and exp != st["pick"]:
        print(f"FAIL pick: expected {exp} got {st['pick']}"); ok = False
    if template_dir:
        mine = fresh_set_from_dir(template_dir, st["prevhash"])
...
ROT = re.compile(r"carousel: rotation h=(\d+) cycle=(\d+) n=(\d+) start=(\d+) skipped=(\d+) idx=(-?\d+) pick=(\S+) set=([0-9a-f]{16}) prevhash=([0-9a-f]{64})")
...
def check_log(path: str, tail: int) -> bool:
    ...
    for h, cycle, n, start, skipped, idx, pick, sh, prevhash in rows:
        n, cycle, start, skipped, idx = int(n), int(cycle), int(start), int(skipped), int(idx)
        s = sets.get(sh)
        if not s or len(s["addrs"]) != s["n"]:
            unknown += 1; continue          # set logged before the tail we read
        addrs = [s["addrs"][i] for i in range(s["n"])]
        exp_start, exp_pick, exp_hash = start_for(prevhash, n), expected_pick(prevhash, addrs, cycle, skipped), set_hash_for(addrs)
        checked += 1
        if exp_hash != sh or exp_start != start or (idx >= 0 and exp_pick != pick):
            ok = False
            print(f"FAIL h={h} cycle={cycle}: logged start={start} pick={pick} set={sh} — expected start={exp_start} pick={exp_pick} set={exp_hash}")
